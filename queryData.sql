...
SELECT synonym_id, COUNT(*) FROM attempts GROUP BY synonym_id;

/* ratio of correct answers all time for synonym
*/
...
/* ratio of correct answers for synonyms top 30 most recent
*/
WITH 
...
/* Average percentage when completed */
SELECT synonym_id, AVG(CAST(correct_num AS REAL) / CAST(total_num AS REAL)) AS ratio FROM completed_games GROUP BY synonym_id ORDER BY ratio;

/* Most recent complet percentage */
...
LEFT JOIN attempts AS helper
 	ON attempts.synonym_id = helper.synonym_id AND attempts.timestamp < helper.timestamp
GROUP BY attempts.synonym_id, attempts.result, attempts.timestamp
HAVING COUNT(helper.timestamp) < 20
ORDER BY attempts.id, attempts.timestamp DESC;
...
/* Number of times word is played */
SELECT word_id, COUNT(*) FROM attempts GROUP BY word_id;

/* words all time ration correct*/
...
/* ratio of correct answers for words top 10 most recent
*/
WITH 
